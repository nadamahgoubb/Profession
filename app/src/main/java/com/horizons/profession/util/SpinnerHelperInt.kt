...
class SpinnerHelperInt(private val context: Context, list: List<String>, hasHint: Boolean) {
    ...
    fun setAdapter(spinner: Spinner) {
        ...
        for (i in list.indices) {
            var s: String = list[i]
           (adapter as ArrayAdapter<String?>).add(s.toString())
         }
    ...
    inner class onItemSelected : AdapterView.OnItemSelectedListener {
     ...
     fun getIndexOf( value: String): Int {
        var index = 0
        val count: Int? = adapter?.count
        if(count!= null ){
        while (index < count) {
            var s: String = list[index]
